...
from src.wildberries.upd_articles import update_user_article, NotSubExcpetion
...
async def get_articles(
    ...
    except NotSubExcpetion:
        raise HTTPException(status_code=402, detail="Для использования сервиса требуется подписка.")
    except TypeError as err:
        raise HTTPException(status_code=401, detail="У вас некорректно настроен токен WB")
...
@app.get("/error/no-token")
async def no_token_error(request: Request):
    return templates.TemplateResponse(
        "error.html",
        {"request": request},
        status_code=401
